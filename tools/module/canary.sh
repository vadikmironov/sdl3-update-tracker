...
expect "oracle: the same defines pass; the order, the spaces, an #undef, the xkbcommon version and the libXi gesture check do not count" 0 "$here/diff_config.sh" "$tmp/same.h" "$oracle"
